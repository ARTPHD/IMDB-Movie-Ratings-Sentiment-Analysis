...
######################### IMPORT GBC MODEL ####################################
###############################################################################
modelsvm = pickle.load(open("IMDb_svm.pkl", "rb"))
...
def sentiment_prediction(text):
    predict_sentiment = modelsvm.predict([text])
...
def main():
    ...
    st.title("IMDB Movie Ratings Sentiment Analysis")
    st.markdown("**Using the state-of-the-art machine learning algorithm Support Vector Machine (SVM) to predict sentiment outcome on IMDB Movie Ratings**")
    st.image("sts.jpeg")
    ...
    text = st.text_area("Enter a series of text or tweets: ", value = "", placeholder = "Enter your IMDb Ratings text/ tweets here (e.g. This Movie Is Good.).")
